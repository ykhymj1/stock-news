...
async function loadV22ExtraInfo(item) {
  ...
  // 🛡️ 안전성 보장 - ticker가 반드시 string 6자리여야 함
  const ticker = String(item && item.ticker ? item.ticker : '').trim();
  console.log('[loadV22ExtraInfo] ticker:', ticker, 'item:', item);
  
  if (!ticker) {
    console.warn('[V22 ExtraInfo] ticker 없음');
    container.style.display = 'none';
    return;
  }
  
  // 6자리 영숫자가 아니면 (object stringify 되어 [object Object] 같은 경우)
  if (!/^[0-9A-Z]{6}$/i.test(ticker)) {
    console.warn('[V22 ExtraInfo] 잘못된 ticker:', ticker);
    container.innerHTML = `
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:14px;text-align:center;color:#dc2626;font-size:12px;">
        ⚠️ 종목 코드 형식 오류: <code style="font-family:monospace;">${escapeHtml(ticker)}</code>
      </div>
    `;
    return;
  }
  ...
  // 1. DART 회사 정보 + 재무 (Worker 직접 호출 - 시크릿 모드도 OK)
  try {
    const dartData = await fetchV22DartFinance(ticker);
    if (dartData) {
...
// 🆕 Worker /dart-finance 직접 호출 (dartKey 없어도 작동)
async function fetchV22DartFinance(ticker) {
  if (!ticker) return null;
  if (!/^[0-9A-Z]{6}$/i.test(ticker)) return null;
  ...
  // 캐시 확인 (24시간)
  try {
    const cacheKey = `v22_dart_${ticker}`;
    const cached = localStorage.getItem(cacheKey);
    if (cached) {
      const parsed = JSON.parse(cached);
      if (parsed.savedAt && (Date.now() - parsed.savedAt) < 24 * 60 * 60 * 1000) {
        console.log('[V22 DART] 캐시 사용:', ticker);
        return parsed.data;
      }
    }
  } catch (e) {}
  
  // apikey가 있으면 사용, 없으면 Worker가 env.DART_API_KEY 사용
  const dartKey = (typeof STATE !== 'undefined' && STATE.settings && STATE.settings.dartKey) 
    ? STATE.settings.dartKey 
    : '';
  
  let url = newsProxy.replace(/\/$/, '') + '/dart-finance?ticker=' + encodeURIComponent(ticker);
  if (dartKey) url += '&apikey=' + encodeURIComponent(dartKey);
  
  try {
    const r = await fetch(url, { signal: AbortSignal.timeout(15000) });
    if (!r.ok) {
      console.warn('[V22 DART] HTTP', r.status, ticker);
      return null;
    }
    const data = await r.json();
    if (!data.ok) {
      console.warn('[V22 DART]', ticker, data.error || 'No data');
      return null;
    }
    ...
    console.warn('[V22 DART] fetch fail', ticker, e.message);
    return null;
  }
}
